  #include<stdio.h>
  #ifdef LLBMC
  #include <llbmc.h>
  #else
 #include <klee/klee.h>
  #endif
  
  #define BOUND 4 //BOUND 5
  int kappa;
  int input,output;
  #include <assert.h>
  #include <math.h>
  #include <stdlib.h>
  
  extern void __VERIFIER_error(int);
  
  // inputs
  int inputs[] = {9,10,4,3,1,2,6,7,5,8};
  
  void calculate_output(int);
  void calculate_outputm1(int);
  void calculate_outputm2(int);
  void calculate_outputm3(int);
  void calculate_outputm4(int);
  void calculate_outputm5(int);
  void calculate_outputm6(int);
  void calculate_outputm7(int);
  void calculate_outputm8(int);
  void calculate_outputm9(int);
  void calculate_outputm10(int);
  void calculate_outputm11(int);
  void calculate_outputm12(int);
  void calculate_outputm13(int);
  void calculate_outputm14(int);
  void calculate_outputm15(int);
  void calculate_outputm16(int);
  void calculate_outputm17(int);
  void calculate_outputm18(int);
  void calculate_outputm19(int);
  void calculate_outputm20(int);
  void calculate_outputm21(int);
  void calculate_outputm22(int);
  void calculate_outputm23(int);
  
  int a1941258847,a831222578,a693035124,a958540235,a2122713545,a1706270929,a1096375983,cf,a459982839,a1236594067;

  
  void calculate_outputm1(int input) {
printf("POINT: 1\n");

  if(((( cf == 1  && (input == 3)) && a459982839 == 34) && a1236594067 == 9)) {
printf("POINT: 2\n");
  cf = 0;
  a1236594067 = 14;
  a2122713545 = 15;
  printf("%d\n", 18); //fflush(stdout);
printf("POINT: 3\n");

  } 
 if((((a1236594067 == 9 &&  cf == 1 ) && (input == 4)) && a459982839 == 34)) {
printf("POINT: 4\n");
  cf = 0;
  a958540235 = 36 ;
  a1236594067 = 11;
  printf("%d\n", 26); //fflush(stdout);
printf("POINT: 5\n");

  } 
 if((((a1236594067 == 9 &&  cf == 1 ) && (input == 10)) && a459982839 == 34)) {
printf("POINT: 6\n");
  cf = 0;
  a1236594067 = 14;
  a2122713545 = 13;
  printf("%d\n", 24); //fflush(stdout);
  }
  }
  void calculate_outputm2(int input) {
printf("POINT: 7\n");

  if((a831222578 == 10 && (( cf == 1  && a1236594067 == 10) && (input == 4)))) {
printf("POINT: 8\n");
  cf = 0;
  a958540235 = 35 ;
  a1236594067 = 11;
  printf("%d\n", 22); //fflush(stdout);
printf("POINT: 9\n");

  } 
 if((a831222578 == 10 && ((input == 7) && (a1236594067 == 10 &&  cf == 1 )))) {
printf("POINT: 10\n");
  cf = 0;
  a1236594067 = 14;
  a2122713545 = 16;
  printf("%d\n", 23); //fflush(stdout);
  }
  }
  void calculate_outputm3(int input) {
printf("POINT: 11\n");

  if((a831222578 == 13 && (a1236594067 == 10 && ((input == 10) &&  cf == 1 )))) {
printf("POINT: 12\n");
  cf = 0;
  
  printf("%d\n", 19); //fflush(stdout);
  }
  }
  void calculate_outputm4(int input) {
printf("POINT: 13\n");

  if(((( cf == 1  && (input == 10)) && a831222578 == 15) && a1236594067 == 10)) {
printf("POINT: 14\n");
  cf = 0;
  a1941258847 = 35 ;
  a1236594067 = 16;
  printf("%d\n", 17); //fflush(stdout);
  }
  }
  void calculate_outputm5(int input) {
printf("POINT: 15\n");

  if((a1236594067 == 11 && (a958540235 == 33 && ( cf == 1  && (input == 2))))) {
printf("POINT: 16\n");
  cf = 0;
  a1236594067 = 10;
  a831222578 = 10;
  printf("%d\n", 18); //fflush(stdout);
printf("POINT: 17\n");

  } 
 if((a1236594067 == 11 && ((input == 3) && (a958540235 == 33 &&  cf == 1 )))) {
printf("POINT: 18\n");
  cf = 0;
  a459982839 = 34 ;
  a1236594067 = 9;
  printf("%d\n", 18); //fflush(stdout);
printf("POINT: 19\n");

  } 
 if(((a958540235 == 33 && ((input == 4) &&  cf == 1 )) && a1236594067 == 11)) {
printf("POINT: 20\n");
  cf = 0;
  a1236594067 = 14;
  a2122713545 = 16;
  printf("%d\n", 23); //fflush(stdout);
  }
  }
  void calculate_outputm6(int input) {
printf("POINT: 21\n");

  if(((((input == 7) &&  cf == 1 ) && a958540235 == 35) && a1236594067 == 11)) {
printf("POINT: 22\n");
  cf = 0;
  a1236594067 = 10;
  a831222578 = 10;
  printf("%d\n", 18); //fflush(stdout);
  }
  }
  void calculate_outputm7(int input) {
printf("POINT: 23\n");

  if(((input == 4) && (a1236594067 == 11 && (a958540235 == 36 &&  cf == 1 )))) {
printf("POINT: 24\n");
  cf = 0;
  a1941258847 = 33 ;
  a1236594067 = 16;
  printf("%d\n", 20); //fflush(stdout);
printf("POINT: 25\n");

  } 
 if(((a958540235 == 36 && ( cf == 1  && a1236594067 == 11)) && (input == 7))) {
printf("POINT: 26\n");
  cf = 0;
  a459982839 = 34 ;
  a1236594067 = 9;
  printf("%d\n", 26); //fflush(stdout);
printf("POINT: 27\n");

  } 
 if(((a958540235 == 36 && (a1236594067 == 11 &&  cf == 1 )) && (input == 10))) {
printf("POINT: 28\n");
  cf = 0;
  
  printf("%d\n", 20); //fflush(stdout);
  }
  }
  void calculate_outputm8(int input) {
printf("POINT: 29\n");

  if((a693035124 == 9 && ((a1236594067 == 12 &&  cf == 1 ) && (input == 1)))) {
printf("POINT: 30\n");
  cf = 0;
  
  printf("%d\n", 20); //fflush(stdout);
printf("POINT: 31\n");

  } 
 if(((input == 8) && ((a693035124 == 9 &&  cf == 1 ) && a1236594067 == 12))) {
printf("POINT: 32\n");
  cf = 0;
  a1236594067 = 14;
  a2122713545 = 15;
  printf("%d\n", 18); //fflush(stdout);
  }
  }
  void calculate_outputm9(int input) {
printf("POINT: 33\n");

  if(((input == 8) && ((a1236594067 == 12 &&  cf == 1 ) && a693035124 == 13))) {
printf("POINT: 34\n");
  cf = 0;
  a1941258847 = 36 ;
  a1236594067 = 16;
  printf("%d\n", 26); //fflush(stdout);
  }
  }
  void calculate_outputm10(int input) {
printf("POINT: 35\n");

  if(((input == 1) && (a1236594067 == 12 && (a693035124 == 15 &&  cf == 1 )))) {
printf("POINT: 36\n");
  cf = 0;
  a1236594067 = 10;
  a831222578 = 13;
  printf("%d\n", 18); //fflush(stdout);
  }
  }
  void calculate_outputm11(int input) {
printf("POINT: 37\n");

  if(((input == 2) && (a1706270929 == 33 && (a1236594067 == 13 &&  cf == 1 )))) {
printf("POINT: 38\n");
  cf = 0;
  a1236594067 = 14;
  a2122713545 = 14;
  printf("%d\n", 25); //fflush(stdout);
printf("POINT: 39\n");

  } 
 if((a1706270929 == 33 && ((input == 4) && (a1236594067 == 13 &&  cf == 1 )))) {
printf("POINT: 40\n");
  cf = 0;
  
  printf("%d\n", 20); //fflush(stdout);
printf("POINT: 41\n");

  } 
 if(((input == 8) && (a1236594067 == 13 && ( cf == 1  && a1706270929 == 33)))) {
printf("POINT: 42\n");
  cf = 0;
  a1236594067 = 14;
  a2122713545 = 11;
  printf("%d\n", 24); //fflush(stdout);
  }
  }
  void calculate_outputm12(int input) {
printf("POINT: 43\n");

  if((a1236594067 == 14 && (( cf == 1  && a2122713545 == 11) && (input == 2)))) {
printf("POINT: 44\n");
  cf = 0;
  a1941258847 = 34 ;
  a1236594067 = 16;
  printf("%d\n", 21); //fflush(stdout);
printf("POINT: 45\n");

  } 
 if((a1236594067 == 14 && (a2122713545 == 11 && ((input == 7) &&  cf == 1 )))) {
printf("POINT: 46\n");
  cf = 0;
  a2122713545 = 16;
  printf("%d\n", 18); //fflush(stdout);
  }
  }
  void calculate_outputm13(int input) {
printf("POINT: 47\n");

  if((((a2122713545 == 13 &&  cf == 1 ) && (input == 4)) && a1236594067 == 14)) {
printf("POINT: 48\n");
  cf = 0;
  a1236594067 = 10;
  a831222578 = 15;
  printf("%d\n", 19); //fflush(stdout);
printf("POINT: 49\n");

  } 
 if(((((input == 8) &&  cf == 1 ) && a1236594067 == 14) && a2122713545 == 13)) {
printf("POINT: 50\n");
  cf = 0;
  a1236594067 = 10;
  a831222578 = 13;
  printf("%d\n", 19); //fflush(stdout);
printf("POINT: 51\n");

  } 
 if(((input == 10) && (a1236594067 == 14 && ( cf == 1  && a2122713545 == 13)))) {
printf("POINT: 52\n");
  cf = 0;
  a2122713545 = 14;
  printf("%d\n", 21); //fflush(stdout);
  }
  }
  void calculate_outputm14(int input) {
printf("POINT: 53\n");

  if((a2122713545 == 14 && ((a1236594067 == 14 &&  cf == 1 ) && (input == 1)))) {
printf("POINT: 54\n");
  cf = 0;
  
  printf("%d\n", 18); //fflush(stdout);
printf("POINT: 55\n");

  } 
 if(((((input == 4) &&  cf == 1 ) && a2122713545 == 14) && a1236594067 == 14)) {
printf("POINT: 56\n");
  cf = 0;
  a459982839 = 34 ;
  a1236594067 = 9;
  printf("%d\n", 26); //fflush(stdout);
  }
  }
  void calculate_outputm15(int input) {
printf("POINT: 57\n");

  if(((a1236594067 == 14 && ((input == 7) &&  cf == 1 )) && a2122713545 == 15)) {
printf("POINT: 58\n");
  cf = 0;
  a1096375983 = 33 ;
  a1236594067 = 15;
  printf("%d\n", 26); //fflush(stdout);
printf("POINT: 59\n");

  } 
 if((a2122713545 == 15 && ((input == 8) && ( cf == 1  && a1236594067 == 14)))) {
printf("POINT: 60\n");
  cf = 0;
  a459982839 = 34 ;
  a1236594067 = 9;
  printf("%d\n", 24); //fflush(stdout);
  }
  }
  void calculate_outputm16(int input) {
printf("POINT: 61\n");

  if(((input == 3) && (a1236594067 == 14 && ( cf == 1  && a2122713545 == 16)))) {
printf("POINT: 62\n");
  cf = 0;
  a1941258847 = 36 ;
  a1236594067 = 16;
  printf("%d\n", 26); //fflush(stdout);
printf("POINT: 63\n");

  } 
 if(((a2122713545 == 16 && (a1236594067 == 14 &&  cf == 1 )) && (input == 4))) {
printf("POINT: 64\n");
  cf = 0;
  a958540235 = 36 ;
  a1236594067 = 11;
  printf("%d\n", 20); //fflush(stdout);
  }
  }
  void calculate_outputm17(int input) {
printf("POINT: 65\n");

  if((((input == 8) && ( cf == 1  && a1096375983 == 32)) && a1236594067 == 15)) {
printf("POINT: 66\n");
  cf = 0;
  a1096375983 = 36 ;
  printf("%d\n", 18); //fflush(stdout);
  }
  }
  void calculate_outputm18(int input) {
printf("POINT: 67\n");

  if((a1236594067 == 15 && ((input == 2) && ( cf == 1  && a1096375983 == 33)))) {
printf("POINT: 68\n");
  cf = 0;
  a1236594067 = 12;
  a693035124 = 9;
  printf("%d\n", 19); //fflush(stdout);
printf("POINT: 69\n");

  } 
 if(((( cf == 1  && a1236594067 == 15) && (input == 7)) && a1096375983 == 33)) {
printf("POINT: 70\n");
  cf = 0;
  a958540235 = 33 ;
  a1236594067 = 11;
  printf("%d\n", 23); //fflush(stdout);
  }
  }
  void calculate_outputm19(int input) {
printf("POINT: 71\n");

  if(((( cf == 1  && a1236594067 == 15) && (input == 7)) && a1096375983 == 36)) {
printf("POINT: 72\n");
  cf = 0;
  a1236594067 = 14;
  a2122713545 = 11;
  printf("%d\n", 26); //fflush(stdout);
printf("POINT: 73\n");

  } 
 if(((a1236594067 == 15 && (a1096375983 == 36 &&  cf == 1 )) && (input == 8))) {
printf("POINT: 74\n");
  cf = 0;
  a1096375983 = 32 ;
  printf("%d\n", 16); //fflush(stdout);
  }
  }
  void calculate_outputm20(int input) {
printf("POINT: 75\n");

  if(((input == 1) && (a1941258847 == 33 && ( cf == 1  && a1236594067 == 16)))) {
printf("POINT: 76\n");
  cf = 0;
  a958540235 = 36 ;
  a1236594067 = 11;
  printf("%d\n", 20); //fflush(stdout);
printf("POINT: 77\n");

  } 
 if((a1941258847 == 33 && (((input == 4) &&  cf == 1 ) && a1236594067 == 16))) {
printf("POINT: 78\n");
  cf = 0;
  a459982839 = 34 ;
  a1236594067 = 9;
  printf("%d\n", 24); //fflush(stdout);
printf("POINT: 79\n");

  } 
 if(((( cf == 1  && a1941258847 == 33) && (input == 6)) && a1236594067 == 16)) {
printf("POINT: 80\n");
  cf = 0;
  a1096375983 = 36 ;
  a1236594067 = 15;
  printf("%d\n", 18); //fflush(stdout);
  }
  }
  void calculate_outputm21(int input) {
printf("POINT: 81\n");

  if((a1236594067 == 16 && (a1941258847 == 34 && ((input == 1) &&  cf == 1 )))) {
printf("POINT: 82\n");
  cf = 0;
  a1236594067 = 12;
  a693035124 = 15;
  printf("%d\n", 23); //fflush(stdout);
printf("POINT: 83\n");

  } 
 if(((input == 3) && (a1941258847 == 34 && ( cf == 1  && a1236594067 == 16)))) {
printf("POINT: 84\n");
  cf = 0;
  a1706270929 = 33 ;
  a1236594067 = 13;
  printf("%d\n", 20); //fflush(stdout);
printf("POINT: 85\n");

  } 
 if(((( cf == 1  && (input == 8)) && a1236594067 == 16) && a1941258847 == 34)) {
printf("POINT: 86\n");
  cf = 0;
  a459982839 = 34 ;
  a1236594067 = 9;
  printf("%d\n", 26); //fflush(stdout);
  }
  }
  void calculate_outputm22(int input) {
printf("POINT: 87\n");

  if(((input == 3) && (( cf == 1  && a1941258847 == 35) && a1236594067 == 16))) {
printf("POINT: 88\n");
  cf = 0;
  
  printf("%d\n", 25); //fflush(stdout);
  }
  }
  void calculate_outputm23(int input) {
printf("POINT: 89\n");

  if((((input == 2) && ( cf == 1  && a1941258847 == 36)) && a1236594067 == 16)) {
printf("POINT: 90\n");
  cf = 0;
  a1096375983 = 36 ;
  a1236594067 = 15;
  printf("%d\n", 26); //fflush(stdout);
printf("POINT: 91\n");

  } 
 if((a1236594067 == 16 && (( cf == 1  && a1941258847 == 36) && (input == 8)))) {
printf("POINT: 92\n");
  cf = 0;
  a459982839 = 34 ;
  a1236594067 = 9;
  printf("%d\n", 18); //fflush(stdout);
  }
  }
  
  void calculate_output(int input) {
  cf = 1;
  
printf("POINT: 93\n");

  if(( cf == 1  && a1236594067 == 9)) {
printf("POINT: 94\n");
printf("POINT: 95\n");

  if(( cf == 1  && a459982839 == 34)) {
printf("POINT: 96\n");
  calculate_outputm1(input);
  }
  }
printf("POINT: 97\n");

  if(( cf == 1  && a1236594067 == 10)) {
printf("POINT: 98\n");
printf("POINT: 99\n");

  if(( cf == 1  && a831222578 == 10)) {
printf("POINT: 100\n");
  calculate_outputm2(input);
  }
printf("POINT: 101\n");

  if((a831222578 == 13 &&  cf == 1 )) {
printf("POINT: 102\n");
  calculate_outputm3(input);
  }
printf("POINT: 103\n");

  if((a831222578 == 15 &&  cf == 1 )) {
printf("POINT: 104\n");
  calculate_outputm4(input);
  }
  }
printf("POINT: 105\n");

  if((a1236594067 == 11 &&  cf == 1 )) {
printf("POINT: 106\n");
printf("POINT: 107\n");

  if(( cf == 1  && a958540235 == 33)) {
printf("POINT: 108\n");
  calculate_outputm5(input);
  }
printf("POINT: 109\n");

  if((a958540235 == 35 &&  cf == 1 )) {
printf("POINT: 110\n");
  calculate_outputm6(input);
  }
printf("POINT: 111\n");

  if((a958540235 == 36 &&  cf == 1 )) {
printf("POINT: 112\n");
  calculate_outputm7(input);
  }
  }
printf("POINT: 113\n");

  if(( cf == 1  && a1236594067 == 12)) {
printf("POINT: 114\n");
printf("POINT: 115\n");

  if((a693035124 == 9 &&  cf == 1 )) {
printf("POINT: 116\n");
  calculate_outputm8(input);
  }
printf("POINT: 117\n");

  if((a693035124 == 13 &&  cf == 1 )) {
printf("POINT: 118\n");
  calculate_outputm9(input);
  }
printf("POINT: 119\n");

  if((a693035124 == 15 &&  cf == 1 )) {
printf("POINT: 120\n");
  calculate_outputm10(input);
  }
  }
printf("POINT: 121\n");

  if(( cf == 1  && a1236594067 == 13)) {
printf("POINT: 122\n");
printf("POINT: 123\n");

  if((a1706270929 == 33 &&  cf == 1 )) {
printf("POINT: 124\n");
  calculate_outputm11(input);
  }
  }
printf("POINT: 125\n");

  if((a1236594067 == 14 &&  cf == 1 )) {
printf("POINT: 126\n");
printf("POINT: 127\n");

  if((a2122713545 == 11 &&  cf == 1 )) {
printf("POINT: 128\n");
  calculate_outputm12(input);
  }
printf("POINT: 129\n");

  if(( cf == 1  && a2122713545 == 13)) {
printf("POINT: 130\n");
  calculate_outputm13(input);
  }
printf("POINT: 131\n");

  if((a2122713545 == 14 &&  cf == 1 )) {
printf("POINT: 132\n");
  calculate_outputm14(input);
  }
printf("POINT: 133\n");

  if((a2122713545 == 15 &&  cf == 1 )) {
printf("POINT: 134\n");
  calculate_outputm15(input);
  }
printf("POINT: 135\n");

  if(( cf == 1  && a2122713545 == 16)) {
printf("POINT: 136\n");
  calculate_outputm16(input);
  }
  }
printf("POINT: 137\n");

  if(( cf == 1  && a1236594067 == 15)) {
printf("POINT: 138\n");
printf("POINT: 139\n");

  if((a1096375983 == 32 &&  cf == 1 )) {
printf("POINT: 140\n");
  calculate_outputm17(input);
  }
printf("POINT: 141\n");

  if(( cf == 1  && a1096375983 == 33)) {
printf("POINT: 142\n");
  calculate_outputm18(input);
  }
printf("POINT: 143\n");

  if(( cf == 1  && a1096375983 == 36)) {
printf("POINT: 144\n");
  calculate_outputm19(input);
  }
  }
printf("POINT: 145\n");

  if((a1236594067 == 16 &&  cf == 1 )) {
printf("POINT: 146\n");
printf("POINT: 147\n");

  if((a1941258847 == 33 &&  cf == 1 )) {
printf("POINT: 148\n");
  calculate_outputm20(input);
  }
printf("POINT: 149\n");

  if((a1941258847 == 34 &&  cf == 1 )) {
printf("POINT: 150\n");
  calculate_outputm21(input);
  }
printf("POINT: 151\n");

  if((a1941258847 == 35 &&  cf == 1 )) {
printf("POINT: 152\n");
  calculate_outputm22(input);
  }
printf("POINT: 153\n");

  if(( cf == 1  && a1941258847 == 36)) {
printf("POINT: 154\n");
  calculate_outputm23(input);
  }
  }
  
  
printf("POINT: 155\n");

  if((cf == 1)){
printf("POINT: 156\n");
  
  
  }
  //fprintf(stderr, "Invalid input: %d\n", input);
  }
  
  int main()
  {
  kappa = 0;
  // main i/o-loop
  int symb;
printf("POINT: 157\n");

  for (int FLAG=0;FLAG<BOUND;FLAG++){
klee_make_symbolic(&symb, sizeof(int ), "symb"); 

klee_make_symbolic(&a1941258847, sizeof(int ), "a1941258847"); 
klee_make_symbolic(&a831222578, sizeof(int ), "a831222578"); 
klee_make_symbolic(&a693035124, sizeof(int ), "a693035124"); 
klee_make_symbolic(&a958540235, sizeof(int ), "a958540235"); 
klee_make_symbolic(&a2122713545, sizeof(int ), "a2122713545"); 
klee_make_symbolic(&a1706270929, sizeof(int ), "a1706270929"); 
klee_make_symbolic(&a1096375983, sizeof(int ), "a1096375983"); 
klee_make_symbolic(&cf, sizeof(int ), "cf"); 
klee_make_symbolic(&a459982839, sizeof(int ), "a459982839"); 
klee_make_symbolic(&a1236594067, sizeof(int ), "a1236594067"); 


printf("%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d",a1941258847,a831222578,a693035124,a958540235,a2122713545,a1706270929,a1096375983,cf,a459982839,a1236594067);

printf("POINT: 159\n");

  if((symb != 9) && (symb != 10) && (symb != 4) && (symb != 3) && (symb != 1) && (symb != 2) && (symb != 6) && (symb != 7) && (symb != 5) && (symb != 8))
printf("POINT: 160\n");
  printf("This is return -2 \n");
  calculate_output(symb);
  }
 printf("%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d",a1941258847,a831222578,a693035124,a958540235,a2122713545,a1706270929,a1096375983,cf,a459982839,a1236594067); 
  return 0;
  
  }
  
  
  
