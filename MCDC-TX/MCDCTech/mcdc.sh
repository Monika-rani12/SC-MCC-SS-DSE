...
./seqshell-mcdc.sh "$BENCHMARK.c" "$TOOL"
cd ..
...
echo "***Total MCDC Sequence Generation Time" $dtA 
printf "Total MCDC Sequence Generation Time: %d:%02d:%02d:%02.4f\n" $ddA $dhA $dmA $dsA 
echo "***Total MCDC Sequence Generation Time" $dtA >> $BENCHMARK-Time-report.txt
