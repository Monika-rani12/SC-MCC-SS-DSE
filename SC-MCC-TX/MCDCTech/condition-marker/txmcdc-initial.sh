...
echo $BENCHMARK >  "$BENCHMARK-ERROR-LIST-$tool/$(basename $BENCHMARK)"
sed -n '/Error: ASSERTION FAIL: kappa violation /p' klee-new/test******.assert.err >> $BENCHMARK-ERROR-LIST-$tool/$(basename $BENCHMARK)
echo '()' >>  "$BENCHMARK-ERROR-LIST-$tool/$(basename $BENCHMARK)"
